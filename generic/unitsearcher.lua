local unitSearcher = CreateFrame('FRAME')
...
local GetAllUnitIDs, GetUnitID
local api = {
   GetAllUnitIDs = function()
      if type(validUnits) ~= 'table' then
         --this table gets returned and someone might be an idiot with it. ensure that it is in fact a table
         validUnits = {}
      else
   ...
   GetUnitID = function(GUID)
      ...
      --if at this point, then validUnits is no longer valid. Perform a search and cache the results.
      return unitSearcher:GetAllUnitIDs()[GUID]
   end
}
LittleScripts:AddModule("UnitSearcher", unitSearcher, "GENERAL")